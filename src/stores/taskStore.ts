...
const fromDb = (row: Record<string, unknown>): Task => ({
  ...
  usedServices: row.used_services as string[] | undefined,
...
const toDb = (task: Partial<Task>) => ({
  ...
  ...(task.githubUrl !== undefined && { github_url: task.githubUrl || null }),
  ...(task.usedServices !== undefined && { used_services: task.usedServices || [] }),
